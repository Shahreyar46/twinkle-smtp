...
if (current_user_can('manage_options')) {
    ...
    if (isset($_REQUEST['data'])) {
        ...
        $sparkpost_mailer_data = json_decode($data, TRUE);

        foreach( $sparkpost_mailer_data as  $key =>$value ) {      
            $this->admin_class->db->update_mailer_data( $key, $value );
        }

        $this->admin_class->db->update_mailer_data('mailer_form_name', 'sparkpost');

        // Unset all forms data
